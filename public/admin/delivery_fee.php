	...
	if (isset($_POST["delivery_fee"])) {
		...
		if ($restaurant_name == "Select Restaurant") {
			...
			if ($restaurant_name != mysqli_fetch_assoc($result)['restaurant_name']) {
				...
				$_SESSION["name_delivery"] = $restaurant_name;
				$_SESSION["id_delivery"] = $restaurant_id;
				redirect_to("delivery_fee_set.php");
			}
		}
	}
...
?>
<div><br /><br />
	<span style="font: 20px Tahoma; text-align: center; color: white" class="row">Add/Update Delivery Fee =></span><br /><br />
		<form action ="delivery_fee.php" enctype="multipart/form-data" method="post">
			<?php 
			...
			<select name="restaurant_id">
				...
				<div id="appDjob">
					<input type="submit" value="Go" name="delivery_fee" />
				</div>
			</div>
	</form>
</div><!-- closes ret row -->
